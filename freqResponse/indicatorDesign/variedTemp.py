...
def impulseSineSimple(t):
    A = 10
    omega = 70
    signal = A*np.sin(omega*t) + 11
    #signal = 200
    return signal
...
if __name__=="__main__":
    ...
    # plots square impulse, use this format for things called on individal time moments.
    #pulse = np.zeros(len(sol.t))
    #for i in range(len(sol.t)):
    #    pulse[i] = impulseSquare(sol.t[i])
    #axs[0].plot(sol.t, pulse)
    
    #pulse = np.zeros(len(sol.t))
    #for i in range(len(sol.t)):
    #    pulse[i] = impulseSquareSingle(sol.t[i])
    #axs[0].plot(sol.t, pulse)

    pulse = np.zeros(len(sol.t))
    for i in range(len(sol.t)):
        pulse[i] = impulseSineSimple(sol.t[i])
    #axs[0].plot(sol.t, pulse)


    # plots simple sine impulse
    plt.suptitle(r"Dynamics, $\omega = 0.5$", fontsize=18)
    axs[0].plot(sol.t, pulse)
    ...
    axs[1].title.set_text("Calcium Ion")#, fontsize = 16)

    axs[2].plot(sol.t, sol.y[1, :])
    axs[2].set_xlabel(r'$t$', fontsize = 16)
    axs[2].set_ylabel(r'$CI^*$', fontsize=16)
    axs[2].title.set_text("Calcium Indicator")#, fontsize = 16)


    print("CI_sim* baseline:", sol.y[0, -1])
