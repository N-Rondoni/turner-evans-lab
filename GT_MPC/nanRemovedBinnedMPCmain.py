...
if __name__=="__main__":
    ...
    # 2 has nans for testing, row 2 subsetAmount = 1000 
    #row = int(sys.argv[1]) 
    CI_Meas = data1[row, :] # looks at a single neuron.  
    #CI_Meas = 50*CI_Meas

    print("preNaN", np.shape(CI_Meas))
    print(np.shape(spikeDatRaw))

    naninds = np.isnan(CI_Meas) | np.isnan(spikeDatRaw)
    print(np.shape(naninds))
    CI_Meas = CI_Meas[~naninds]
    spikeDatRaw = spikeDatRaw[~naninds]
    
    print("postNan", np.shape(CI_Meas))
    print(np.shape(spikeDatRaw))
