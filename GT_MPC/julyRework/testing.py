...
def expAverage(signal):
    sFactor = 0.5
    # signal to be smoothed, sFactor the smoothing factor 0 < sFactor < 1
    sigOut = np.zeros(np.shape(signal))
    sigOut[0] = signal[0]
    for i in range(1, np.max(np.shape(signal))):
        sigOut[i] = sFactor*signal[i] + (1-sFactor)*signal[i-1]
    return sigOut
...
if __name__=="__main__":
    ...
    # uncomment hardcoded subsetAmount to test smaller subsets
...
#    Ci = model.set_variable('_x', 'Ci') #
    ...
    #lambd = model.set_variable('_x', 'lambd')
...
#    print(model.u.keys())
    ...
    #mterm = ((model.x['CiF']-baseLine)/baseLine - model.tvp['Ci_m'])**2 + model.x['lambd']*(model.u['s'] != 0)   
...
#    print("Simulated Data shape:", np.shape(mpc.data['_x']), t_f[-1])
#    print("Shape of original data:", np.shape(timeVec), timeVec[-1])
#    print("shape of sol:", np.shape(s))
