...
#    Ci_f = mpc.data['_x'][:, 1]
    ...
    mpc_graphics.add_line(var_type='_x', var_name='CiF', axis=ax[1])
    mpc_graphics.add_line(var_type='_u', var_name='s', axis=ax[2])
    ax[0].set_ylabel(r'$Ca^{2+}$')
    ax[1].set_ylabel(r'$CI^*$')
    ax[2].set_ylabel('$s$')
    ...
    #print(len(mpc_graphics.pred_lines['_x', :, :,0]))
    #
    #print(np.shape(mpc_graphics.pred_lines['_x', :, :,0]))
    #print(np.shape(mpc_graphics.pred_lines['_x', :, :,0]))
    #
    ...
    def update(t_ind):
        ...
        #window_size = 100
        #min_time = max(0, t_ind - window_size // 2)
        #max_time = min(n_steps - 1, t_ind + window_size // 2)
    ...
    # can pull specific frames too
    #specific_frames = list(range(1050, 1650))
    #anim = FuncAnimation(fig, update, frames=specific_frames, blit=True)
