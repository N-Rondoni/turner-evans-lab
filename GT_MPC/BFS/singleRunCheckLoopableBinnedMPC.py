...
#    Ci = model.set_variable('_x', 'Ci') #
    ...
    kf = float(sys.argv[5])    #0.0513514
    kr = float(sys.argv[6])    # 7.6 
    alpha = float(sys.argv[3]) # 20 
    gamma = float(sys.argv[4]) # 0.1 
    baseLine = float(sys.argv[7]) # 2.5 was nice for row 2
...
#   model.set_rhs('Ci', kr*CiF - kf*Ci*Ca)
    ...
    #print("file " + str(row) + " " + str(dset) + " " + str(alpha) + " " + str(gamma) + " " + str(kf) + " " + str(kr) + " " + str(baseLine)) # + parameters
...
#    os.system('cp ' + filename + ' /mnt/c/Users/nicho/Pictures/MPC_CRE_across_nodes/') # only run with this line uncommented if you are Nick
    ...
    np.save('data/rawSignals/s_node_' + str(row) + 'dset_' + str(dset) + 'alpha_' + str(alpha) + 'gamma_' + str(gamma) + 'kf_' + str(kf) + 'kr_' + str(kr) + 'bl_' + str(baseLine), s)
    ...
    file1 = open("data/corrScores.dat", "a")
    file1.write(str(corrCoef) + ' node_' + str(row) + ' dset_' + str(dset) + ' alpha_' + str(alpha) + ' gamma_' + str(gamma) + ' kf_' + str(kf) + ' kr_' + str(kr) + ' bl_' + str(baseLine))
    file1.write("\n")
    file1.close()
