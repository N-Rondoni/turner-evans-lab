...
#    os.system('cp ' + filename + ' /mnt/c/Users/nicho/Pictures/MPC_CRE_across_nodes/') # only run with this line uncommented if you are Nick
    ...
    # plotting 4 Cornell -----------------------------------------------------
    # recreates plots above in the style of orginal paper, nice and stacked.

    # Don't rename just keep usin
    #s = s[subL:subH]
    #spikeDat = spikeDat[subL:subH]
    ...
    #CI_Meas_interp = CI_Meas_interp/np.max(CI_Meas_interp[subL:subH]) 
    ...
    gridAxis = "both"
    axs[0].plot(t_f[subL:subH], CiF_f[subL:subH], label=r'$CI^{*}_{Sim}$', color = color1) ## subtracting baseline
    axs[0].plot(t_f[subL:subH], CI_Meas_interp[subL:subH], label=r'$CI^{*}_{Meas}$', color = color2)
    ...
    # if you want a 3x2 try this
    #fig, axs = plt.subplots(3, 2)
    #axs[0,0].plot(t_f[subL:subH], CiF_f[subL:subH], label=r'$CI^{*}_{Sim}$') ## subtracting baseline
    #axs[0,0].plot(t_f[subL:subH], CI_Meas_interp[subL:subH], label=r'$CI^{*}_{Meas}$', alpha = 0.7)
    #axs[0,0].set_xlabel(r'$t$', fontsize = 16)
    #axs[0,0].set_ylabel(r'$CI^*$', fontsize = 16)
    #axs[0,0].grid()
    #axs[0,0].legend()

    #axs[1,0].plot(newTime[subL:subH], spikeDat[subL:subH], label=r'Recorded spike rate')
    #axs[1,0].set_xlabel(r'$t$', fontsize = 16)
    #axs[1,0].set_ylabel(r'$s$', fontsize = 16)        
    #axs[1,0].grid()
    #plt.legend()

    #axs[2,0].plot(newTime[subL:subH], s[subL:subH], label=r'Simulated rate')
    #axs[2,0].set_xlabel(r'$t$', fontsize = 16)
    #axs[2,0].set_ylabel(r'$s$', fontsize = 16)        
    #axs[2,0].grid()
    #plt.legend()
