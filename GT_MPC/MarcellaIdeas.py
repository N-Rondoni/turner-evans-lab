...
#    Ci = model.set_variable('_x', 'Ci') #
    ...
    if dset in [5, 3, 1]: # params found from BFS, worked well on 5 for sure.
        kf = 0.2
        kr = 10 
        alpha = 10 
        gamma = 0.73333   # passive diffusion
        L = CiF_0 + 100   # total amount of calcium indicator
        baseLine = 0.5 
    
    s = model.set_variable('_u', 's')         
    CI_m = model.set_variable('_tvp', 'Ci_m') 

    X = 2
    model.set_rhs('Ca', alpha*s + kr*CiF - kf*Ca*(L - CiF) - 0.4/(1 + (Ca/X)))
