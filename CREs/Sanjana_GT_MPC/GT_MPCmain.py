...
#Where CI_Meas is used
def tvp_fun(t_now):
    for k in range(n_horizon + 1):
        tvp_template['_tvp', k, 'Ci_m'] = np.interp(t_now + k*t_step, timeVec, CI_Meas) #interpolates data
    return tvp_template

#time varying paramter -sets Ci_m value to be interplated : t_now current ime, timeVec= how frequently there was a recording
def tvp_fun_sim(t_now):
    tvp_template1['Ci_m'] = np.interp(t_now, timeVec, CI_Meas) #interpolating 
...
if __name__=="__main__":
    ...
    # Load in Dan's flouresc. data from janelia. Using cond{1}.allFlyData{1}.Strip{2}.RROIavMax
    #replace with calcium csv
    file_path = 'data/calcium.csv'
    ...
    file_path_2 = 'data/1_test_spikes.csv'
    #mat = scipy.io.loadmat(file_path2)
    data2 = pd.read_csv(file_path_2).T

    t_f = n * (0.04) #Multiply final time with 0.04 seconds because 25 Hz sampling rate... 
                                #"ground truth" may be 100HZ => multiply with 0.01 seconds
    (m1, n1) = data1.shape
    #define time vec same as number of instances/elements as there are columns.
    timeVec = np.linspace(0, t_f , n) 

    
    CI_Meas = np.array(data1.iloc[row, :n]) #Making the CI_meas go up to the same number of columns as timeVec 
                                                # Looks at a single neuron.  #pull out specific row of data set
    ...
    Ci_0 = 70   #CI         
    CiF_0 = CI_Meas[0] #CI_Meas[0]  #CI^* #was previously 0, real data readouts start with some concentration.
...
#    Ca_ext = 100 # constant extracellular calcium.
    ...
    #model.set_meas('s', s)# = model.set_variable
    ...
    setup_mpc = {
            'n_horizon': 100, # pretty short horizion
            't_step': 1/6, # (s)
            'n_robust': 1,
...
#    otherTerm = model.x['CiF']**2 + model.x['Ca']**2 + model.x['Ci']**2
#    lterm = model.u['s']**2 #+ (model.x['CiF'] - model.tvp['Ci_m'])**2 # stage cost 
    lterm = mterm #+ otherTerm
    ...
    # define constraints
    mpc.bounds['lower', '_x', 'Ca'] = -100
    mpc.bounds['lower', '_x', 'Ci'] = -100
    mpc.bounds['lower', '_x', 'CiF'] = -100
    mpc.bounds['lower', '_u', 's'] = -5 # slow diffusion
#    mpc.bounds['upper', '_u', 's'] = 200
    ...
    params_simulator = {
            ...
            'abstol': 1e-10,
            'reltol': 1e-10,
            't_step': 0.04, # (s) mean step is 6.11368547250401 in data
            }
    ...
    n_steps = 700 # may need to do n steps? 
...
#    print(mpc.data['_time'])
    ...
    print(np.shape(mpc.data['_x']))  #This is printing the (700, 3)
...
#    plt.figure(6)
#    print('t shape:', np.shape(t_f))
#    print(t_f[-1])
#    print('s shape:', np.shape(s))


#    spikeDat = np.array(data2.iloc[row, :n])
#    print('timeVec shape:', np.shape(timeVec))
#    print(timeVec[-1])
#    print('spke dat shape:', np.shape(spikeDat))
#    plt.plot(timeVec, spikeDat)
