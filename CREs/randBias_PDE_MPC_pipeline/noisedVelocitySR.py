...
###
#       creates locDif, used in the controlled solution
#
###
...
def weightMat(x, t):
    ...
    # add in randomness CAUSES MAT TO BECOME ILL CONDITIONED
    #Wrand = np.random.rand(*np.shape(W)) # need * operator for some reason
    #randScalar = 1E-2
    
    #Wrand = np.random.normal(0, 0.1, np.shape(W)) #mean, std dev, shape
    #randScalar = 1

    #Wrand = np.ones(np.shape(W))
    #randScalar = 0.01

#  print("Cond with no randomness:", np.linalg.cond(W))
   # W = W + randScalar*Wrand
    
    #print("Cond with randomness:", np.linalg.cond(W))
...
#        velNoise = -1*np.random.uniform(0, 0.5, np.shape(realVel)) #uniformly sample [0, 0.5)
 ...
 #   plot3d(thetaGrid, timeGrid, firingRate)
    ...
    np.save('data/PDEfiringRates_Noised' + state + '.npy', firingRate)
    np.save('data/PDEfiringTimes_Noised' + state + '.npy', sol.t)
